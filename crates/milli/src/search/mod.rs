...
impl<'a> Search<'a> {
    ...
    pub fn build_located_query_terms(
    ...
    ) -> Result<(Option<Vec<new::LocatedQueryTerm>>, Vec<PinDoc>, bool), Error> {
        let mut used_negative_operator = false;

        let mut ignored = RoaringBitmap::new();

        let query_terms = if let Some(query) = self.query.as_deref() {
        ...
        let pins = self
            ...
            .map(|dsrs| dsrs.resolve_pins(query_terms.as_deref().unwrap_or(&[]), universe, ctx))
            .transpose()?
            .unwrap_or_default();

        *universe -= ignored;

        Ok((query_terms, pins, used_negative_operator))
    }
}
